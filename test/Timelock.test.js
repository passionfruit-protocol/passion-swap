...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should not allow non-owner to do operation', async () => {
        await this.passion.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.passion.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.passion.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.passion.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.passion.owner()).valueOf(), carol);
    });

    it('should also work with PassionPrincess', async () => {
