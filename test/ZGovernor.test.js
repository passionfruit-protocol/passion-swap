...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        await this.passion.delegate(dev, { from: dev });
        this.chef = await PassionPrincess.new(this.passion.address, dev, '100', '0', { from: alice });
        ...
        // Perform another deposit to make sure some PSFs are minted in that 1 block.
        await this.chef.deposit(0, '100', { from: minter });
        assert.equal((await this.passion.totalSupply()).valueOf(), '1066');
        assert.equal((await this.passion.balanceOf(minter)).valueOf(), '1000');
        assert.equal((await this.passion.balanceOf(dev)).valueOf(), '66');
        // Transfer ownership to timelock contract
        this.timelock = await Timelock.new(alice, time.duration.days(2), { from: alice });
        this.gov = await GovernorAlpha.new(this.timelock.address, this.passion.address, alice, { from: alice });
